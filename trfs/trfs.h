...
#include "tr_types.h"
extern atomic_t trecord_id;
extern int fill_record(rec_type_t type, struct dentry *dentry, int flags, int perm_flags, int err,
                    void *aux, int aux_len, struct tr_record *record);
extern int append_record(void  *upper_sb, struct tr_record *record);
...
#define TRFS_PAGE(_super_)  (((struct trfs_sb_info*)(_super_)->s_fs_info)->tr_page)
/* file to lower file */
